...
public class CandidateAquisitionReportSector extends BaseClass {
    TestEntity testData;
    int counter = 1;

    public CandidateAquisitionReportSector(TestEntity testData) {
    ...
    public TestResult executeTest() {
        ...
        this.setStartTime();
        if (!CandidateAquisitionReportSector()) {
    ...
    public boolean CandidateAquisitionReportSector(){
        ...
        if (!SeleniumDriverInstance.navigateTo(Main_Object.CandidateAquisitionReportSectorUrl())) {
            return false;
        }

        SeleniumDriverInstance.takeScreenShot("Navigate to CARS", false);

        if (!SeleniumDriverInstance.clickElementbyXpath(Main_Object.DateFromSec())) {
            return false;
        }
        SeleniumDriverInstance.takeScreenShot("Test", false);

        if (!SeleniumDriverInstance.clickElementbyXpath(Main_Object.SourceDrillDownSec())) {
            return false;
        }
        ...
        if (!SeleniumDriverInstance.clickElementbyXpath(Main_Object.TopPostedJobTitles())) {
            return false;
        }
        ...
        if (!SeleniumDriverInstance.clickElementbyXpath(Main_Object.TopSearchedJobdDrillDown())) {
            return false;
        }
